...
void sendRes(int efd, int fd) {
	...
	if(output_log) printf("write bytes left: %lu\n", left);
	...
	if (left == 0) {
		...
		if (con.writeEnabled) {
			...
			if(output_log) printf("write done delete write ev\n");
		}
		if(output_log) printf("erase fd %d\n", fd);
		cons.erase(fd);
		return;
	}
	if (wd < 0 &&  (errno == EAGAIN || errno == EWOULDBLOCK)) {
		if (!con.writeEnabled) {
			...
			if(output_log) printf("write eagain add write ev\n");
		}
		return;
	}
	if (wd<=0) {
